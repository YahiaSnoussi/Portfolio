...
### Copilot Anomaly-Prevention Skill (work)
Idea stage (2026-09-25): turn the team's anomaly knowledge base (Excel, ~55 fixed anomalies from a mainframe-to-Linux batch migration: SORT syntax incompatibilities, VB/FB/RDW file formats, variable substitution, missing binaries/data, EBCDIC-vs-ASCII COBOL issues) into GitHub Copilot customizations: repo instructions + path-specific instructions + Agent Skills + prompt files + a deterministic lint script in CI. Architecture proposed; nothing built yet.
...
- [x] Merge `MasterxWishlist` into `master` — **done 2026-09-25** (fast-forward to `b631fcd`).
- [x] Production code prep — **done 2026-09-26 on branch `production_mig`** (`0bf8f93`): env-var config, Docker Compose + Caddy for **raffaella.tn**, backups, DEPLOY.md; stack tested locally in Docker.
- [ ] Buy VPS + point raffaella.tn DNS, add https://raffaella.tn to Google OAuth origins, first deploy following DEPLOY.md with a NEW `JWT_SECRET`; then merge `production_mig` into `master`.
- [ ] Legal pages (mentions légales, CGV, confidentialité — loi 2004-63) before selling.
...
### Copilot Anomaly-Prevention Skill
- [ ] Get answers from Yahia (repo layout/file extensions, Copilot surfaces used, KB owner) then generate the kit: `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md`, `.github/skills/*`, prompt files, `kb/anomalies.yaml`, lint script
...
- 2026-09-26: Raffaella — `production_mig` branch for raffaella.tn: env-var config (secrets out of git for good), Docker Compose + Caddy auto-HTTPS, backups, French DEPLOY.md; fixed wrong-password 500→401 and missing-file 500→404; full prod stack verified locally in Docker. Earlier: gift-set badge contrast fix, removed placeholder delivery rows from cart/checkout.
- 2026-09-25: Raffaella — fast-forward merged `MasterxWishlist` into `master`; mapped out what's missing for a production deploy (VPS + domain + Docker Compose/Caddy, secrets, localhost fixes, backups, legal pages); caching deferred.
- 2026-09-25: Scoped a GitHub Copilot skill architecture to prevent recurring anomalies from a work migration knowledge base (Excel). Proposal only, no code yet.
