...
### Raffaella
Premium bio-cosmetics e-commerce brand (Tunisian market + international). Spring Boot + Angular, MySQL. Past MVP, now with: product variants + cost price + featured/display-order, package reviews (reviews generalized to cover products AND bundles), coupon unlimited-use threshold, admin delete/search/filter coverage (nav-section delete, search on Categories/Coupons/Packages, multi-filter on Orders), WhatsApp/Twilio removed (email-only notifications now, confirmed actually arriving), a shared admin-modal pattern used across all CRUD screens, wishlist (products + packages, server-persisted), and guest checkout (place an order with no account). This latest work (wishlist commit `9f7eb2b` 2026-09-23, guest checkout commit `d687351` 2026-09-24) is pushed to `origin/MasterxWishlist` — **not yet merged into `master`**, that's a decision still pending with Yahia. Everything through 2026-08-24 (commit `cff79ac`/`05a608f`) is on `master`.
...
- [ ] Manually click through guest checkout in a browser (backend/frontend both verified via compile+build+live curl, but no visual walkthrough done yet)
