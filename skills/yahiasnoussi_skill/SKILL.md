...
### Raffaella
Premium bio-cosmetics e-commerce brand (Tunisian market + international). Spring Boot + Angular, MySQL. Past MVP, now with: product variants + cost price, dynamic DB-driven navbar sections, review moderation (3-state, own admin page + notification bell), a shared admin-modal pattern used across all CRUD screens, package/bundle image upload + real DTOs, Tunisian governorate checkout field, and a homepage-reset admin action. Committed+pushed to `origin/master` 2026-08-22 (commit `976354a`), on top of the full feature set first pushed 2026-08-21.
...
### Raffaella
- [ ] Confirm order notification emails actually arrive — Gmail address + App Password are now filled in locally (`application.properties`, deliberately NOT committed 2026-08-22, still needs Twilio SID/token for WhatsApp) but an actual send hasn't been confirmed yet
...
- [ ] Package product picker still capped at first 100 products, client-side filter only — no real backend search/pagination yet
- [ ] No delete endpoint for navbar sections (create + bulk-visibility-toggle exist, delete doesn't)
...
- [ ] Migrate off `ddl-auto=update` to Flyway/Liquibase before prod — already had to work around one MySQL strict-mode `ALTER TABLE` limitation on a non-empty table (`Review.status`)
